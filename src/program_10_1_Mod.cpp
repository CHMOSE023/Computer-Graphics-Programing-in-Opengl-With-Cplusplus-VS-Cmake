...
int main(void) {
    ...
    GLFWwindow* window = glfwCreateWindow(mode->width, mode->height, " program_10_1 ", nullptr, nullptr);    
    if (!window) {
...
void init(GLFWwindow* window) {
	renderingProgram = Utils::createShaderProgram("./shaders/vertex_shader101.glsl", "./shaders/fragment_shader101.glsl");
	...
	roofTexture = Utils::loadTexture("./textures/castleroofNORMAL.jpg");
	//roofTexture = Utils::loadTexture("./textures/circuits_normal.jpg");
	//roofTexture = Utils::loadTexture("./textures/normalMap2.jpg");
}
...
void display(GLFWwindow* window, double currentTime) {
	...
	float angleZ = toRadians(80.0f);
	mMat = glm::rotate(mMat, angleZ, glm::vec3(0.0f, 1.0f, 0.0f));
	rotAmt = currentTime;
