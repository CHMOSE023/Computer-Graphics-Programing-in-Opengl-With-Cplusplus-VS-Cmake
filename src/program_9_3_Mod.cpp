...
void init(GLFWwindow* window) {
	renderingProgram = Utils::createShaderProgram("./shaders/vertex_shader93Mod.glsl", "./shaders/fragment_shader93Mod.glsl");
...
void display(GLFWwindow* window, double currentTime) {
	...
	mvLoc = glGetUniformLocation(renderingProgram, "mv_matrix");
	projLoc = glGetUniformLocation(renderingProgram, "proj_matrix");
	eyeLoc = glGetUniformLocation(renderingProgram, "eyePos");
	...
	glUniformMatrix4fv(projLoc, 1, GL_FALSE, glm::value_ptr(pMat));
    glUniform3f(eyeLoc, cameraLoc.x, cameraLoc.y, cameraLoc.z);
