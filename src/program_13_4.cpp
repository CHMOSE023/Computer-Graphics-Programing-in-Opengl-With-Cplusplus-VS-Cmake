...
int main(void) {
    ...
    GLFWwindow* window = glfwCreateWindow(mode->width, mode->height, " program_13_4 ", nullptr, nullptr);    
    if (!window) {
...
void init(GLFWwindow* window) {
	renderingProgram = Utils::createShaderProgram("./shaders/vertex_shader134.glsl",
                            "./shaders/geometry_shader134.glsl", "./shaders/fragment_shader134.glsl");
	
    glfwGetFramebufferSize(window, &width, &height);
	aspect = (float)width / (float)height;
	pMat = glm::perspective(1.0472f, aspect, 0.1f, 1000.0f);

	torLocX = 0.0f; torLocY = -0.75f; torLocZ = -1.0f;
...
void display(GLFWwindow* window, double currentTime) {
	...
	mMat = glm::rotate(mMat, toRadians(135.0f), glm::vec3(1.0f, 0.0f, 0.0f));
