...
int main(void) {
    ...
    GLFWwindow* window = glfwCreateWindow(mode->width, mode->height, " program_13_3 ", nullptr, nullptr);    
    if (!window) {
...
void init(GLFWwindow* window) {
	renderingProgram = Utils::createShaderProgram("./shaders/vertex_shader133.glsl",
                            "./shaders/geometry_shader133.glsl", "./shaders/fragment_shader133.glsl");
	
    glfwGetFramebufferSize(window, &width, &height);
	aspect = (float)width / (float)height;
	pMat = glm::perspective(1.0472f, aspect, 0.1f, 1000.0f);

	torLocX = 0.0f; torLocY = 0.00f; torLocZ = 1.0f;
